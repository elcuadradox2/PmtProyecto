...
$query = $db->prepare("SELECT no_licencia FROM agresiones WHERE id= :post_id");
...
$result = $db->prepare("SELECT foto FROM fotos_agresiones WHERE licencia_agresiones= :no_licencia");
...
$result = $db->prepare("DELETE FROM fotos_agresiones WHERE licencia_agresiones= :no_licencia");
$result->bindParam(':no_licencia', $no_licencia);
if(!$result->execute()){    
