...
$query = $db->prepare("SELECT no_licencia, fecha_hora FROM agresiones WHERE id= :post_id");
...
$result = $db->prepare("SELECT foto FROM fotos_agresiones WHERE licencia_agresiones= :no_licencia AND fecha_agresion= :fecha_hora");
...
while($row = $result->fetch(PDO::FETCH_ASSOC)){
    ...
    $ruta_foto = 'files_boletaagresiones/' . $row['foto'];
...
$result = $db->prepare("DELETE FROM fotos_agresiones WHERE licencia_agresiones= :no_licencia AND fecha_agresion= :fecha_hora");
$result->bindParam(':no_licencia', $no_licencia);
$result->bindParam(':fecha_hora', $fecha_hora);
if(!$result->execute()){    
...
$result = $db->prepare("DELETE FROM agresiones WHERE id= :post_id");
