...
$query = $db->prepare("SELECT fecha_hora FROM entrevista WHERE id= :post_id");
...
$result = $db->prepare("SELECT foto FROM fotos_entrevista WHERE fecha_entrevista= :fecha_hora");
$result->bindParam(':fecha_hora', $fecha_hora);
$result->execute();
while($row = $result->fetch(PDO::FETCH_ASSOC)){
    // Añade el nombre de la carpeta a la ruta de la foto
    $ruta_foto = 'files_boletaentrevista/' . $row['foto'];
...
$result = $db->prepare("DELETE FROM fotos_entrevista WHERE fecha_entrevista= :fecha_hora");
$result->bindParam(':fecha_hora', $fecha_hora);
if(!$result->execute()){    
...
$result = $db->prepare("DELETE FROM entrevista WHERE id= :post_id");
$result->bindParam(':post_id', $id);
if($result->execute()){    
    header("location:vistaentrevista.php");
}else{
