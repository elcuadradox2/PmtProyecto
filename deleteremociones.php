...
$query = $db->prepare("SELECT ubicacion_remocion FROM remociones WHERE id= :post_id");
...
$result = $db->prepare("SELECT foto FROM fotos_remociones WHERE ubicaciones_remociones= :ubicacion_remocion");
...
$result = $db->prepare("DELETE FROM fotos_remociones WHERE ubicaciones_remociones= :ubicacion_remocion");
$result->bindParam(':ubicacion_remocion', $ubicacion_remocion);
if(!$result->execute()){    
