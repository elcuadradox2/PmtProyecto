...
$query = $db->prepare("SELECT fecha_hora FROM reporte_interno WHERE id= :post_id");
...
$result = $db->prepare("SELECT foto FROM fotos_reporte WHERE fecha_reporte= :fecha_hora");
$result->bindParam(':fecha_hora', $fecha_hora);
$result->execute();
while($row = $result->fetch(PDO::FETCH_ASSOC)){
    // Añade el nombre de la carpeta a la ruta de la foto
    $ruta_foto = 'files_boletareporteinterno/' . $row['foto'];
...
$result = $db->prepare("DELETE FROM fotos_reporte WHERE fecha_reporte= :fecha_hora");
$result->bindParam(':fecha_hora', $fecha_hora);
if(!$result->execute()){    
...
$result = $db->prepare("DELETE FROM reporte_interno WHERE id= :post_id");
