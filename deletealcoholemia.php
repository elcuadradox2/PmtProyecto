...
$query = $db->prepare("SELECT no_licencia FROM alcoholemia WHERE id= :post_id");
...
$result = $db->prepare("SELECT foto FROM fotos_alcoholemia WHERE licencias_alcoholemia= :no_licencia");
...
$result = $db->prepare("DELETE FROM fotos_alcoholemia WHERE licencias_alcoholemia= :no_licencia");
$result->bindParam(':no_licencia', $no_licencia);
if(!$result->execute()){    
