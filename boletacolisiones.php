...
if (isset($_POST['submit']) && count($_FILES['fotos_colisiones']['name']) > 0) {
    ...
    if (count($_FILES['fotos_colisiones']['name']) > 10) {
        // Redirigir con mensaje de error
        header("Location: colisiones.php?status=error&message=too_many_files");
        exit();
    }
...
header("Location: colisiones.php?status=success");
exit();
?>